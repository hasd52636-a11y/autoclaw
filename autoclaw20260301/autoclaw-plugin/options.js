// AutoClaw options.js - Dual Port Mode Support
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const port18792 = document.getElementById('port18792');
  const port30000 = document.getElementById('port30000');
  ...
  async function loadSettings() {
    ...
    // Port Mode Selection - 默认30000端口
    const portMode = s.portMode || '30000';
    if (portMode === '18792') {
      port18792.checked = true;
    } else if (portMode === 'dual') {
      portDual.checked = true;
    } else {
      port30000.checked = true;
    }

    relayPort.value = s.relayPort || DEFAULT_PORT;
    // 默认勾选Auto Attach All Tabs
    autoAttachAll.checked = s.autoAttachAll !== false; // 默认true
    maxTabs.value = s.maxTabs || 50;

    // Bookmark Classification Settings - 默认Local Analysis (Privacy)
    ...
    const token = s.gatewayToken || (portMode === '30000' ? BUILT_IN_TOKEN : OPENCLAW_TOKEN);
    if (token.length > 20) {
      tokenDisplay.textContent = token.substring(0, 12) + '***' + token.substring(token.length - 8);
    } else {
    ...
    const targetHost = portMode === '30000' ? '127.0.0.1' : '127.0.0.1:18789';
    wsDisplay.textContent = `ws://${targetHost}/extension`;
  ...
  async function checkConnection() {
    try {
      const selectedPort = port30000.checked ? 30000 : 18792;
  ...
  async function saveSettings() {
    ...
    const portMode = portDual.checked ? 'dual' : (port30000.checked ? '30000' : '18792');
    ...
    await chrome.storage.local.set({
      portMode: portMode,
      relayPort: port,
      gatewayToken: portMode === '30000' ? BUILT_IN_TOKEN : OPENCLAW_TOKEN,
  ...
  testBtn.addEventListener('click', async () => {
    if (!verifyAndRun()) return;
    const port = parseInt(relayPort.value) || DEFAULT_PORT;
    const token = port30000.checked ? BUILT_IN_TOKEN : OPENCLAW_TOKEN;
  ...
  port18792.addEventListener('change', () => { 
    relayPort.value = 18792; 
    wsDisplay.textContent = 'ws://127.0.0.1:18789/extension';
    tokenDisplay.textContent = OPENCLAW_TOKEN.substring(0, 12) + '***' + OPENCLAW_TOKEN.substring(OPENCLAW_TOKEN.length - 8);
    checkConnection(); 
  });
  port30000.addEventListener('change', () => { 
    relayPort.value = 30000; 
    wsDisplay.textContent = 'ws://127.0.0.1:30000/extension';
    tokenDisplay.textContent = BUILT_IN_TOKEN.substring(0, 12) + '***' + BUILT_IN_TOKEN.substring(BUILT_IN_TOKEN.length - 8);
    checkConnection(); 
  });
  
  portDual.addEventListener('change', () => { 
    relayPort.value = 30000; 
    wsDisplay.textContent = 'ws://127.0.0.1:30000/extension + ws://127.0.0.1:18789/extension';
    tokenDisplay.textContent = 'MCP: ' + BUILT_IN_TOKEN.substring(0, 12) + '*** + Gateway: ' + OPENCLAW_TOKEN.substring(0, 12) + '***';
    checkConnection(); 
  });

  // 确保在30000模式下使用内置token
  port30000.addEventListener('change', () => { 
    relayPort.value = 30000; 
    wsDisplay.textContent = 'ws://127.0.0.1:30000/extension';
    tokenDisplay.textContent = BUILT_IN_TOKEN.substring(0, 12) + '***' + BUILT_IN_TOKEN.substring(BUILT_IN_TOKEN.length - 8);
